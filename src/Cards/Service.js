...
const Service = (props) => {
  return (
    <div className=" min-h-[400px] max-w-[300px] shadow-lg shadow-[#2a2185] hover:shadow-xl hover:shadow-[#2a2185] rounded-lg flex flex-col items-center">
        ...
        <button><NavLink to={`/username/provider/yourservices/:${props.items.id}`}>Details</NavLink></button>
        <button><NavLink to={`/username/services/:${props.items.id}`}>Details</NavLink></button>
    </div>
  )
}
